...
class Product
{
  ...
  function add($args)
  {
    ...
    $args['sku'] = strtoupper($args['sku']);
    $args['category_id'] = $args['category_id'];
  ...
  function update($args)
  {
    ...
    $obj = $this->model();
    $count = $obj->getListBy(['sku' => $args['sku']], ['id' => $args['id']]);
    if (count($count) > 0) {
      return ['status' => 'error', 'msg' => 'This code/sku already exist', 'data' => $count];
    }
  ...
  function delete($args)
  {
    ...
    $insertData = $obj->updateByMeta(['deleted' => 1], ['id' => $args['id'], 'sku' =>  $args['sku']]);
    if ($insertData === false) {
      return ['status' => 'error', 'msg' => 'Error in delete product', 'sql' => $obj->db->sql];
    }
